...
class AddressService extends Service {
  async getAddressSummary(addressIds, p2pkhAddressIds, rawAddresses) {
    const {Address} = this.ctx.app.nccinfo.lib
    ...
    let [
      ...
      nrc20Service.getAllNRC20Balances(hexAddresses),
      nrc721Service.getAllQRC721Balances(hexAddresses),
  ...
  async getAddressTransactionCount(addressIds, rawAddresses) {
    const {Address: RawAddress, Solidity} = this.app.nccinfo.lib
  ...
  async getAddressNRC20TokenTransactionCount(rawAddresses, token) {
    const {Address, Solidity} = this.app.nccinfo.lib
    const TransferABI = Solidity.nrc20ABIs.find(abi => abi.name === 'Transfer')
  ...
  async getAddressNRC20TokenTransactions(rawAddresses, token) {
    const {Address, Solidity} = this.app.nccinfo.lib
    const TransferABI = Solidity.nrc20ABIs.find(abi => abi.name === 'Transfer')
    const db = this.ctx.model
    const {sql} = this.ctx.helper
    ...
    let topicAddresses = rawAddresses
      .filter(address => address.type === Address.PAY_TO_PUBLIC_KEY_HASH)
      .map(address => Buffer.concat([Buffer.alloc(12), address.data]))
    let totalCount = await this.getAddressNRC20TokenTransactionCount(rawAddresses, token)
    let transactions = await db.query(sql`
      SELECT
      ...
      INNER JOIN nrc20 ON nrc20.contract_address = log.address
  ...
  async getAddressNRC20TokenMempoolTransactions(rawAddresses, token) {
        ...
        },
        {
          ...
          include: [{
            ...
            include: [{
              ...
              as: 'contract',
              required: true,
              where: {address: token.contractAddress, type: 'nrc20'},
              attributes: []
            }]
          }]
        },
