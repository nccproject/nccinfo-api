...
class NRC20Service extends Service {
  async listNRC20Tokens() {
    const db = this.ctx.model
    const {Nrc20Statistics: NRC20Statistics} = db
    const {sql} = this.ctx.helper
    const {gt: $gt} = this.app.Sequelize.Op
    let {limit, offset} = this.ctx.state.pagination

    let totalCount = await NRC20Statistics.count({
    ...
    let list = await db.query(sql`
      SELECT
        ...
        nrc20.name AS name, nrc20.symbol AS symbol, nrc20.decimals AS decimals, nrc20.total_supply AS totalSupply,
        nrc20.version AS version,
        list.holders AS holders,
        list.transactions AS transactions
      FROM (
        SELECT contract_address, holders, transactions FROM nrc20_statistics
        WHERE transactions > 0
        ORDER BY transactions DESC
        LIMIT ${offset}, ${limit}
      ) list
      INNER JOIN nrc20 USING (contract_address)
  ...
  async getAllNRC20Balances(hexAddresses) {
    ...
    const {
      ...
      Contract, EvmReceipt: EVMReceipt, Nrc20: NRC20, Nrc20Balance: NRC20Balance,
      where, col
    } = this.ctx.model
    const {in: $in} = this.app.Sequelize.Op
    let list = await NRC20.findAll({
    ...
    let mapping = new Map(list.map(item => [
      item.contract.addressString,
      {
        ...
        symbol: item.symbol,
        decimals: item.decimals,
        balance: item.contract.nrc20Balances.map(({balance}) => balance).reduce((x, y) => x + y),
        unconfirmed: {
          received: 0n,
          sent: 0n
        }
      ...
      if (byteCode.length === 68
        ...
        if (mapping.has(item.output.address.contract.addressString)) {
          ...
          data = {
            ...
            name: item.output.address.contract.nrc20.name,
            symbol: item.output.address.contract.nrc20.symbol,
            decimals: item.output.address.contract.nrc20.decimals,
            balance: 0n,
            unconfirmed: {
              received: 0n,
              sent: 0n
  ...
  async getNRC20Balance(rawAddresses, tokenAddress) {
    const {Address: RawAddress, OutputScript, Solidity} = this.app.nccinfo.lib
    const transferABI = Solidity.nrc20ABIs.find(abi => abi.name === 'transfer')
    const {
      Address, TransactionOutput,
      Contract, EvmReceipt: EVMReceipt, Nrc20: NRC20, Nrc20Balance: NRC20Balance,
    ...
    let token = await NRC20.findOne({
      where: {contractAddress: tokenAddress},
      attributes: ['name', 'symbol', 'decimals'],
      transaction: this.ctx.state.transaction
    })
    let list = await NRC20Balance.findAll({
  ...
  async getNRC20BalanceHistory(addresses, tokenAddress) {
    ...
    const {
      ...
      Contract, Nrc20: NRC20, Nrc20Balance: NRC20Balance,
      literal
    } = db
    ...
    let [{totalCount}] = await db.query(sql`
      ...
      FROM evm_receipt receipt, evm_receipt_log log, nrc20
      WHERE receipt._id = log.receipt_id AND log.address = nrc20.contract_address AND ${{raw: logFilter}}
    `, {type: db.QueryTypes.SELECT, transaction: this.ctx.state.transaction})
    if (totalCount === 0) {
      return {totalCount: 0, transactions: []}
    }
    let ids = (await db.query(sql`
      ...
      INNER JOIN (
        SELECT DISTINCT(receipt.transaction_id) AS id FROM evm_receipt receipt, evm_receipt_log log, nrc20
        WHERE receipt._id = log.receipt_id AND log.address = nrc20.contract_address AND ${{raw: logFilter}}
    ...
    if (list.length > 0) {
      let intialBalanceList = await NRC20Balance.findAll({
        where: {
      ...
      for (let log of logs) {
        ...
        if (item) {
          ...
          result.tokens.push({
            ...
            name: log.nrc20.name.toString(),
            symbol: log.nrc20.symbol.toString(),
  ...
  async getAllNRC20TokenTransactions() {
    const TransferABI = this.app.nccinfo.lib.Solidity.nrc20ABIs.find(abi => abi.name === 'Transfer')
    const db = this.ctx.model
    const {sql} = this.ctx.helper
    let {limit, offset, reversed = true} = this.ctx.state.pagination
    ...
    let [{totalCount}] = await db.query(sql`
      ...
      FROM nrc20, evm_receipt_log log
      WHERE nrc20.contract_address = log.address AND log.topic1 = ${TransferABI.id}
    `, {type: db.QueryTypes.SELECT, transaction: this.ctx.state.transaction})
    let transactions = await db.query(sql`
      SELECT
        ...
        nrc20.name AS name,
        nrc20.symbol AS symbol,
        nrc20.decimals AS decimals,
        evm_receipt_log.topic2 AS topic2,
        evm_receipt_log.topic3 AS topic3,
        evm_receipt_log.data AS data
      FROM (
        SELECT log._id AS _id FROM nrc20, evm_receipt_log log
        WHERE nrc20.contract_address = log.address AND log.topic1 = ${TransferABI.id}
      ...
      INNER JOIN nrc20 ON nrc20.contract_address = evm_receipt_log.address
  ...
  async getNRC20TokenTransactions(contractAddress) {
    const TransferABI = this.app.nccinfo.lib.Solidity.nrc20ABIs.find(abi => abi.name === 'Transfer')
    const db = this.ctx.model
  ...
  async getNRC20TokenRichList(contractAddress) {
    const db = this.ctx.model
    const {Nrc20Balance: NRC20Balance} = db
    const {ne: $ne} = this.app.Sequelize.Op
    let {limit, offset} = this.ctx.state.pagination

    let totalCount = await NRC20Balance.count({
  ...
  async updateNRC20Statistics() {
    const TransferABI = this.app.nccinfo.lib.Solidity.nrc20ABIs.find(abi => abi.name === 'Transfer')
    const db = this.ctx.model
    const {Nrc20: NRC20, Nrc20Statistics: NRC20Statistics} = db
    const {sql} = this.ctx.helper
    let transaction = await db.transaction()
    try {
      let result = (await NRC20.findAll({attributes: ['contractAddress'], transaction})).map(
        ({contractAddress}) => ({contractAddress, holders: 0, transactions: 0})
      )
      let balanceResults = await db.query(sql`
        SELECT contract_address AS contractAddress, COUNT(*) AS count FROM nrc20_balance
      ...
      await db.query(sql`DELETE FROM nrc20_statistics`, {transaction})
      await NRC20Statistics.bulkCreate(result, {validate: false, transaction, logging: false})
